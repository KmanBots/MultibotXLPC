...
export var CollectionControl = L.Control.extend({
  ...
  onAdd: function (map) {
    ...
    var container = L.DomUtil.create(
      "div",
      "leaflet-bar leaflet-control noselect"
    );
    ...
    // TODO: Lets have this be selected on boot
    // Area control
    this._createControl(
    ...
    // this._toggleCollectionMode(this._areas, "areas_converter", this);
    ...
    $("#code-output").on("input propertychange paste", () =>
      context._loadFromText()
    );
  ...
  _drawMouseArea: function (e) {
    ...
    if (this._currentDrawable instanceof Areas) {
      if (this._firstSelectedAreaPosition !== undefined) {
        ...
        this._drawnMouseArea = new Area(
          this._firstSelectedAreaPosition,
          mousePos
        ).toLeaflet(this._map);
  ...
  _outputCode: function () {
    ...
    //   console.log(("I wonder what the default botAPI is? " botAPI));

      console.log(this._currentDrawable)

      // TODO: Send to backend

      output = converters['TRiBot'][this._currentConverter].toJava(
        this._currentDrawable
      );
    }
  ...
  _loadFromText: function () {
    if (this._currentDrawable !== undefined) {
      var botAPI = $("#bot-api option:selected").text();
      converters[botAPI][this._currentConverter].fromJava(
        $("#code-output").text(),
        this._currentDrawable
      );
    }
